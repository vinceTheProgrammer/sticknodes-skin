<template>
  <div class="card card-compact bg-secondary shadow-xl md:col-span-1">
    <div class="card-body">
      <button class="btn">
        <font-awesome-icon icon="fa-solid fa-circle-xmark" /> Clear Filters
      </button>
      <ul class="w-full mx-0">
        <li v-for="category in getFilters()" :key="category.label">
...
function getFilters() {
  if (route.name) {
    switch (route.name) {
      case "browse-skins":
        return filterCategories.skins;
      case "browse-users":
        return filterCategories.users;
      default:
        return filterCategories.default;
    }
  } else {
    return filterCategories.default;
  }
